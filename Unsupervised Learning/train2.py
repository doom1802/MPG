...
#The SummaryWriter class is your main entry to log data for visualization by TensorBoard.
    ...
    for epoch in range(args.num_epochs):
        ...
        tq.set_description('epoch %d, lr %f, lr_discriminator %f' % (epoch, lr, lr1))
        loss_record_source = []
        loss_record_target = []
        ...
        for batch_train, batch_target in zip(enumerate(trainloader), enumerate(targetloader)):
          ...
          # train Segmentation network
            ...
            with amp.autocast():
                ...
                loss_seg_target = args.lambda_adv_target * loss_adv_target

            scaler.scale(loss_seg_target).backward()
          ...
          # train Discriminator network
            ...
            tq.set_postfix(loss_seg_source='%.6f' % loss_seg_source, loss_seg_target='%.6f' % loss_seg_target)
            step += 1
            writer.add_scalar('loss_step', loss_seg_source, step)
            writer.add_scalar('loss_step', loss_seg_target, step)
            
            loss_record_source.append(loss_seg_source.item())
            loss_record_target.append(loss_seg_target.item())

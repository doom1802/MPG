...
def get_label_info(json_path):
  ...
  ann = json.load(f)
  label = {}
  for lbl, palette, label2train in zip(ann["label"], ann["palette"], ann["label2train"]):
    label_name = lbl
    r = palette[0]
    g = palette[1]
    b = palette[2]
    index = label2train[0]
    class_index = label2train[1]

    label[label_name] = [int(r), int(g), int(b), int(index), int(class_index)]
  f.close()
  return label
...
def one_hot_it_v11(label, label_info):
  ...
  for index, info in enumerate(label_info):
    ...
    class_index = label_info[info][4]
    class_20 = label_info[info][3]
    if class_20 != 255:
      #color_map = np.full(label.shape, color, dtype=int)
      equality = np.equal(label, color)
      class_map = np.all(equality, axis=-1)
			# semantic_map[class_map] = index
